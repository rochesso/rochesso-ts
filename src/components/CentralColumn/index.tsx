...
const CentralColumn = ({onClickHandler, leftColumnIsOpen, location}: AppProps) => {
    ...
    if (leftColumnIsOpen) {
        arrowButton =
            <img className={styles.arrow} onClick={LeftColumnIsOpenHandler} src={leftArrow} alt="Close sidebar"/>;
    } else {
        arrowButton =
            <img className={styles.arrow} onClick={LeftColumnIsOpenHandler} src={rightArrow} alt="Open sidebar"/>;
    }
    ...
    return <ul className={styles.list}>
        ...
        {MENU_LINKS.map((link) => {
                key++;
                return <NavItem location={location} leftColumnIsOpen={leftColumnIsOpen} onClickHandler={onClickHandler}
                                key={key} to={link.to}
                                title={link.title}/>;
